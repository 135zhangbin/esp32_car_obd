...
#define GATTS_DEMO_CHAR_VAL_MAX 500

#define HEART_PROFILE_NUM                         1
#define HEART_PROFILE_APP_IDX                     0
#define ESP_HEART_RATE_APP_ID                     0x55
#define EXAMPLE_DEVICE_NAME                       "ESP_BLE_SECURITY3"
...
static uint8_t sec_service_uuid[16] = {  
    /* LSB <--------------------------------------------------------------------------------> MSB */  
    //first uuid, 16bit, [12],[13] is the value  
    0x01, 0x35, 0x23, 0x82, 0x90, 0x00, 0x00, 0x80, 0x00, 0x10, 0x00, 0x00, 0x18, 0x23, 0x03, 0x04,  
};


// config adv data 配置广播数据
...
// config scan response data 配置扫描响应数据
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event,
                                        esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param); //处理gatt事件
...
static  uint8_t body_sensor_loc_val[1] = {0x02};
...
static  uint8_t heart_ctrl_point[100] = {0x02};
//static const uint8_t heart_ctrl_point[1] = {0x00};

/// Full HRS Database Description - Used to add attributes into the database
static  esp_gatts_attr_db_t heart_rate_gatt_db[HRS_IDX_NB] =
{
    ...
    // Heart Rate Control Point Characteristic Declaration   特征的元数据，例如权限和属性
    [HRS_IDX_HR_CTNL_PT_CHAR]          =
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_declaration_uuid, ESP_GATT_PERM_READ,
      CHAR_DECLARATION_SIZE,CHAR_DECLARATION_SIZE, (uint8_t *)&char_prop_read_write}},

    // Heart Rate Control Point Characteristic Value 特征的当前值
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&heart_rate_ctrl_point, ESP_GATT_PERM_WRITE_ENCRYPTED|ESP_GATT_PERM_READ_ENCRYPTED,
      GATTS_DEMO_CHAR_VAL_MAX, sizeof(heart_ctrl_point), (uint8_t *)heart_ctrl_point}},
};

void sendData(char send_data_str[]){
  // 转换为 uint8_t 数组
   uint8_t *send_data = (uint8_t *)send_data_str;
   // 计算数据长度
   uint16_t send_data_len = strlen(send_data_str);
   // 发送数据
   esp_err_t res = esp_ble_gatts_set_attr_value(heart_rate_handle_table[HRS_IDX_HR_CTNL_PT_VAL], send_data_len, send_data);
   if (res != ESP_OK) {
    // 错误处理
       ESP_LOGE("", "Error occurred: %s", esp_err_to_name(res));
  }else{
        //    ESP_LOGE("", "correct");
  }
    // uint8_t send_data[] = {0x01};
    // esp_err_t res= esp_ble_gatts_set_attr_value(heart_rate_handle_table[HRS_IDX_HR_CTNL_PT_VAL], sizeof(send_data), (uint8_t*)send_data);
    // if (res != ESP_OK) {
    // ESP_LOGE("", "Error occurred: %s", esp_err_to_name(res));
    // }
}
                                        ...
                                        esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    switch (event) {
        ...
        case ESP_GATTS_READ_EVT:
             ESP_LOGI(GATTS_TABLE_TAG, "ESP_GATTS_READ_EVT, read value:");
            break;
                                ...
                                esp_ble_gatts_cb_param_t *param)
{
    ...
    do {
        ...
        for (idx = 0; idx < HEART_PROFILE_NUM; idx++) {
            //检查当前的 GATT 接口是否与传递给事件处理器的 GATT 接口匹配。如果是，或者 GATT 接口为 ESP_GATT_IF_NONE（表示没有指定特定的 GATT 接口）
...
void blue_tooth_start(void)
{
    ...
    ESP_ERROR_CHECK(esp_bt_controller_mem_release(ESP_BT_MODE_CLASSIC_BT));  //释放芯片内存
    ...
    ret = esp_ble_gatts_register_callback(gatts_event_handler);  //注册gatt回调
    if (ret){
        ESP_LOGE(GATTS_TABLE_TAG, "gatts register error, error code = %x", ret);
        return;
    }
    ret = esp_ble_gap_register_callback(gap_event_handler);//注册gap
    if (ret){
  ...
  char example_data[] = "No Data";  // 替换为您要传递的数据
